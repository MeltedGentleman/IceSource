	...
	if (In.size() != 0) {
		...
		if (tolower(In.at(0)) == "select" || tolower(In.at(0)) == "box") {
			if (tolower(In.at(1)) == "me") {
				...
				rlua_getfield(luaState, -1, "LocalPlayer");
				rlua_getfield(luaState, -1, "Character");
				rlua_getfield(luaState, -1, "SelectionBox");

				rlua_getfield(luaState, LUA_GLOBALSINDEX, "Color3");
				rlua_getfield(luaState, -1, "new");
				rlua_pushnumber(luaState, 225);
				rlua_pushnumber(luaState, 0);
				rlua_pushnumber(luaState, 0);
				rlua_pcall(luaState, 3, 1, 0);
				rlua_setfield(luaState, -7, "Color3");*/
		...
		if (tolower(In.at(0)) == "fencingr" || tolower(In.at(0)) == "freach") {
			...
			game.Players.LocalPlayer.Backpack.Foil.Handle.CanCollide=false
			...
			*/
			...
			//rlua_pushstring(luaState, "false");
			//rlua_setfield(luaState, -2, "CanCollide");
...
void luaC(std::string input) {
	try {
		...
		if (In.at(0) == "getfield") {
			rlua_getfield(luaState, stoi(In.at(1)), In.at(2).c_str());
		}
